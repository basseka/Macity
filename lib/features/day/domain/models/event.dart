...
class Event with _$Event {
  ...
  const factory Event({
    ...
    @JsonKey(name: 'dates_affichage_horaires')
    @Default('')
    String datesAffichageHoraires,
    ...
    @JsonKey(name: 'categorie_de_la_manifestation')
    @Default('')
    String categorie,
    @JsonKey(name: 'theme_de_la_manifestation') @Default('') String theme,
    @Default('') String rubrique,
    @JsonKey(name: 'manifestation_gratuite')
    @Default('')
    String manifestationGratuite,
    @JsonKey(name: 'tarif_normal') @Default('') String tarifNormal,
    @JsonKey(name: 'reservation_site_internet')
    @Default('')
    String reservationUrl,
    @JsonKey(name: 'reservation_telephone')
    @Default('')
    String reservationTelephone,
    @JsonKey(name: 'station_metro_tram_a_proximite')
    @Default('')
    String stationProximite,
    ...
    // Coordonnées du lieu (source OpenAgenda). 0 = inconnu → l'itinéraire
    // « Ma soirée » retombe alors sur une sélection par ville.
