...
final sportSubcategoryCountProvider =
  ...
  // Rugby and Cette Semaine have their own logic inside fetchSupabaseMatches
  final matches =
  ...
  if (searchTag == 'Cette Semaine') {
    return matches.where(_isKnownSport).length;
  }
...
final sportMatchesProvider = FutureProvider<List<SupabaseMatch>>((ref) async {
  ...
  if (subcategory == 'Cette Semaine') {
    return matches.where(_isKnownSport).toList();
  }
  return matches;
});
